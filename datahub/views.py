...
from datahub.pipelines.hub import Api_Request
...
def execute(request):
  if request.method == 'POST':
    req: Api_Request = Api_Request(json.loads(request.body))
    action = req.action
    reference = Generic_Reference.from_dict(req.data)
    ...
    if action == 'create':
      try:
        if check_permission_create(reference, request.user):
          ...
          if (action_performed ):
            return Api_Response(200, "Succesfuly created element with", [Generic_Reference.from_dict(fetched_data)]).JsonResponse()
        else:
          return Api_Response(403, "Error: User is not allowed to perform this action", None).JsonResponse()
      except Exception as e:
        e.print_exc()
        return Api_Response(500, "Could not create element\n error-message: "+str(e), None).JsonResponse()
      ...
      try:
        if check_permission_update(reference, request.user):
          ...
          if (action_performed):
            return Api_Response(200, "Succesfuly updated element with id = "+str(reference.id), None).JsonResponse()
        else:
          return Api_Response(403, "Error: User is not allowed to perform this action", None).JsonResponse()
      except Exception as e:
        e.print_exc()
        return Api_Response(500, "Could not update element with id = "+str(reference.id)+"\n error-message: "+str(e), None).JsonResponse()
      ...
      try:
        if check_permission_fetch_all(reference, request.user):
          ...
          if (not fetched_data is None):
            return Api_Response(200, "Succesfuly retrieved elements", fetched_data).JsonResponse()
        else:
          return Api_Response(403, "Error: User is not allowed to perform this action", fetched_data).JsonResponse()
      except Exception as e:
        e.print_exc()
        return Api_Response(404, "No data found with this query role = "+reference.role+"\n error-message: "+str(e), None).JsonResponse()
      ...
      try:
        if check_permission_fetch(reference, request.user):
          ...
          if (not fetched_data is None):
            return Api_Response(200, "Succesfuly retrieved element with id = "+str(reference.id), fetched_data).JsonResponse()
        else:
          return Api_Response(403, "Error: User is not allowed to perform this action", fetched_data).JsonResponse()
      except Exception as e:
        e.print_exc()
        return Api_Response(404, "No data found with this query id = "+reference.id+"\n error-message: "+str(e), None).JsonResponse()
      ...
      try:
        if check_permission_delete(reference, request.user):
          ...
          if (action_performed):
            return Api_Response(200, "Succesfuly retrieved element with id = "+str(reference.id), None).JsonResponse()
        else:
          return Api_Response(403, "Error: User is not allowed to perform this action", None).JsonResponse()
      except Exception as e:
        e.print_exc()
        return Api_Response(500, "Could not delete element with id = "+reference.id+"\n error-message: "+str(e), None).JsonResponse()
    else:
