    ...
    if action == 'create':
      try:
        if check_permission_create(reference, request.user):
          ...
          if (action_performed ):
            return ApiResponse(200, "Succesfuly created element with", [Generic_Reference.from_dict(fetched_data)]).json_response()
        else:
      ...
      try:
        if check_permission_update(reference, request.user):
          ...
          if (action_performed):
            return ApiResponse(200, "Succesfuly updated element with id = "+str(reference.id), None).json_response()
        else:
          return ApiResponse(403, "Error: User is not allowed to perform this action", None).json_response()
      except Exception as e:
        e.print_exc()
      ...
      try:
        if check_permission_fetch_all(reference, request.user):
          fetched_data = pipe.fetch_all_generic(reference)
      ...
      try:
        if check_permission_fetch(reference, request.user):
          ...
          if (not fetched_data is None):
            return ApiResponse(200, "Succesfuly retrieved element with id = "+str(reference.id), fetched_data).json_response()
        else:
          return ApiResponse(403, "Error: User is not allowed to perform this action", fetched_data).json_response()
      except Exception as e:
        e.print_exc()
      ...
      try:
        if check_permission_delete(reference, request.user):
          ...
          if (action_performed):
            return ApiResponse(200, "Succesfuly retrieved element with id = "+str(reference.id), None).json_response()
        else:
          return ApiResponse(403, "Error: User is not allowed to perform this action", None).json_response()
      except Exception as e:
        e.print_exc()
